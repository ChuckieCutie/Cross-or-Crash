...
// export function Tree(tileIndex, height = 25) {
...
//         new THREE.CylinderGeometry(branchRadius * 0.3, branchRadius, branchLength, 6),
//         new THREE.MeshLambertMaterial({
//           color: "#654321",
//           flatShading: true,
//         })
...
// export function Tree(tileIndex, season = 'spring', height = 25) {
//   const tree = new THREE.Group();
//   tree.position.x = tileIndex * tileSize;
...
//         new THREE.MeshLambertMaterial({
//           color: ['#FFB6C1', '#FFC0CB', '#FF69B4'][Math.floor(Math.random() * 3)]
//         })
//       );
//       flower.position.set(
//         (Math.random() - 0.5) * 30,
...
//         new THREE.MeshLambertMaterial({
//           color: ['#FF6347', '#FF4500', '#DAA520'][Math.floor(Math.random() * 3)],
//           transparent: true,
//           opacity: 0.8
//         })
//       );
...
export function FruitTree(tileIndex, fruitType = 'apple', height = 25) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    puff.scale.set(0.6 + Math.random() * 0.3, 0.6 + Math.random() * 0.3, 0.6 + Math.random() * 0.3);
    puff.position.set(
      (Math.random() - 0.5) * 25,
      (Math.random() - 0.5) * 25,
      (Math.random() - 0.5) * 8
...
export function AnimatedTree(tileIndex, height = 25) {
  ...
  tree.animate = function(time) {
    ...
    tree.children.forEach((child, index) => {
      if (child.userData) {
        const sway = Math.sin(time * windSpeed + child.userData.animationOffset) * windStrength;
        const heightFactor = child.position.z / height; // Higher parts sway more
        
        child.rotation.x = child.userData.originalRotation.x + sway * heightFactor * 0.1;
        child.rotation.z = child.userData.originalRotation.z + sway * heightFactor * 0.05;
      }
    });
  };
...
export function Tree(tileIndex, height = 25, type = 'original') {
  switch(type) {
    case 'realistic': return RealisticTree(tileIndex, height);
    case 'spring': return SeasonalTree(tileIndex, 'spring', height);
    case 'summer': return SeasonalTree(tileIndex, 'summer', height);
    case 'autumn': return SeasonalTree(tileIndex, 'autumn', height);
    case 'winter': return SeasonalTree(tileIndex, 'winter', height);
    case 'apple': return FruitTree(tileIndex, 'apple', height);
    case 'orange': return FruitTree(tileIndex, 'orange', height);
    case 'cherry': return FruitTree(tileIndex, 'cherry', height);
    case 'animated': return AnimatedTree(tileIndex, height);
    case 'dead': return DeadTree(tileIndex, height);
    default:
      // Original tree code
      const tree = new THREE.Group();
      tree.position.x = tileIndex * tileSize;
